...
{   
	var vars = new Array();      // два вспомагательных 
	var pair = new Array();     // массива 
	var param = new Array(); 
			
	var query = location.search;  // строка GET запроса 
	if(query != '') 
	{ 
		vars = (query.substr(1)).split('&');   // разделяем переменные 
		for(var i=0; i < vars.length; i++) 
		{ 
			pair = vars[i].split('=');       // массив param будет содержать 
